...
from theme import apply_theme, sidebar_brand, COLORS, severity_badge, logout_button
...
with tab_blue:
    for ev in get_blue_team_evaluations():
        with st.container(border=True):
            c1, c2, c3 = st.columns([2, 0.8, 3])
            c1.markdown(f"<span style='font-weight:600; font-size:14px;'>{ev['category']}</span>", unsafe_allow_html=True)
            c2.markdown(f"<span style='font-family:JetBrains Mono,monospace; font-size:14px;'>{ev['score']}</span>", unsafe_allow_html=True)
            c3.markdown(severity_badge(ev["severity"]), unsafe_allow_html=True)
            st.markdown(f"<div style='color:{COLORS['text_secondary']}; font-size:13px; margin-top:6px;'>{ev['note']}</div>", unsafe_allow_html=True)
